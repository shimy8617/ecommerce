<?php
	...
	if(isset($_FILES['imagen'])){
		...
		$sql="update producto set nompro='$nompro',despro='$despro', estado=$estado,prepro=$prepro,rutimapro='$nombre_imagen'
		where codpro=$codpro";
		$result=mysqli_query($con,$sql);
		if ($result) {
			if (move_uploaded_file($_FILES['imagen']['tmp_name'],"../../catalogo-online-aypoo/assets/products/".$nombre_imagen)) {
					$response->state=true;
					unlink("../../catalogo-online-aypoo/assets/products/".$rutimapro);
			}else{
